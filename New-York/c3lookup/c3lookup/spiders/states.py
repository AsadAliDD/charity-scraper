...
class StatesSpider(scrapy.Spider):
    ...
    start_urls = ['http://501c3lookup.org/Alaska/']
    ...
    def parse(self, response):
        ...
        next_page_link='http://501c3lookup.org/Alaska/'+l
    ...
    def parse_charity(self,response):
        ...
        # Income
        income=response.xpath("//h5[contains(text(), 'INCOME AMOUNT')]/span/text()").extract_first()        
        assets=response.xpath("//h5[contains(text(), 'ASSET AMOUNT')]/span/text()").extract_first()
